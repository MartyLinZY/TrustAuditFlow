...
EXPECTED_PAYLOAD_TYPES = [
    "blop_event_integrity_anchor",
    "blop_governance_anchor",
    "trustflowaudit_strategy_anchor",
    "erasure_recovery_anchor",
    "artifact_manifest_anchor",
]


def load_result(result_dir: Path) -> dict[str, Any]:
    result_path = result_dir / "audit_chain_results.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify BLoP audit-chain anchoring outputs.")
    ...
    parser.add_argument("--expect-artifacts", type=int)
    args = parser.parse_args()

    result = load_result(Path(args.result_dir))
    ...
    expect_equal("artifact_count", result.get("artifact_count"), args.expect_artifacts)
    expect_equal("payload_types", result.get("payload_types"), EXPECTED_PAYLOAD_TYPES)
    expect_equal("payload_count", result.get("payload_count"), len(EXPECTED_PAYLOAD_TYPES))
    if len(blocks) != len(EXPECTED_PAYLOAD_TYPES):
        raise AssertionError(f"blocks={len(blocks)}, expected={len(EXPECTED_PAYLOAD_TYPES)}")
    if not result.get("simulation_result_hash"):
        raise AssertionError("missing simulation_result_hash")

    prev_hash = "GENESIS"
    for expected_height, row in enumerate(blocks, start=1):
        if int(row["height"]) != expected_height:
            raise AssertionError(f"height mismatch at {expected_height}")
        ...
        if row["payload_type"] != EXPECTED_PAYLOAD_TYPES[expected_height - 1]:
            raise AssertionError(f"payload type mismatch at height {expected_height}")
